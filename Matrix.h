...
namespace Numeric_lib {
    ...
    public:
...

        {
            if (i == j) return;
            Index max = this->dim1();
            for (Index ii = 0; ii < max; ++ii) std::swap((*this)(ii, i), (*this)(ii, j));
        }
    ...
    // if (A[j].slice(j).size() != A[i].slice(j).size()) error("sizes wrong for scale_and_add()");
                  //Numeric_lib::Matrix<double> res(A[j].slice(j).size());
                 // for (Index k = 0; k < n; ++k) res[k] += A[j].slice(j)[k] * -mult + A[i].slice(j)[k];
                 // A[i].slice(j) = res.xfer();
                  ////double s= b(i);
                //for (Index i = 0; i < A[i].slice(i + 1).size(); ++i) s += A[i].slice(i + 1)[i] * x.slice(i + 1)[i]
