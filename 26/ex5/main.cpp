...
{
    return a.label==b.label && a.val == b. val && a.seq == b.seq && a.res == b.res;
}
//------------------------------------------------------------------------------
...
int test_all_from_file(const string& file)
{
    ...
    while (ifs >> t)
    {
        Test<T> t_dup; t_dup.label=t.label; t_dup.val=t.val;  t_dup.seq=t.seq;  t_dup.res=t.res; 

        cout <<"Testing:"<< t << endl;
        bool r = bin_search(t.seq.begin(), t.seq.end(), t.val);
        if(!(t_dup == t))
            cout<<"binary search changes the sequnece\n";
        if (r != t.res)
        {
